...
//Boats have queued up at both sides of the lock and your managers are asking for an update
//on how long it's going to take for all the boats to go through the lock.
...
{
    private static readonly uint LockFillingTimeInMinutes = 2;
    public static uint CanalMania(uint[] lowQueue, uint[] highQueue, uint lockLength)
    {
        ...
        LockPosition lockPosition = LockPosition.Low;

        while (lowPos < lowQueue.Length || highPos < highQueue.Length)
        {
            if (lockPosition == LockPosition.Low)
            {
                uint t = 0;
                while (lowPos < lowQueue.Length && t + lowQueue[lowPos] <= lockLength)
                {
                    t += lowQueue[lowPos++];
                }

                totalMinutes += 2 * t + LockFillingTimeInMinutes;
                lockPosition = LockPosition.High;
            }
            else
            {
                uint t = 0;
                while (highPos < highQueue.Length && t + highQueue[highPos] <= lockLength)
                {
                    t += highQueue[highPos++];
                }

                totalMinutes += 2 * t + LockFillingTimeInMinutes;
                lockPosition = LockPosition.Low;
            }
        }

        if (lockPosition == LockPosition.High)
        {
            totalMinutes += 2;
        }
